(ns cs_disco.main
  "Kotodama WASM entrypoint for the Cs_disco clean-room actor (L4) — Clojure port.
  ...
  facts (`cs_disco.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cs_disco")
(def tier "L4")
...
(def entity-specs
  [{:entity "Matter"   :plural "matters"   :id-prefix "csdisco_mat"
    :fields [:clientId :name :status :openedAt]              :required [:name :status]
    :coerce {}                       :refs {}}
   {:entity "Document"      :plural "documents"      :id-prefix "csdisco_doc"
...
(defn emit-facts
  "EAVT facts for one record: {\"cs_disco.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cs_disco-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cs_disco-compat/ready)
